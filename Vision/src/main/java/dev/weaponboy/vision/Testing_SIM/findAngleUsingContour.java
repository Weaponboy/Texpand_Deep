...
public class findAngleUsingContour implements VisionProcessor{
    ...
    public Object processFrame(Mat frame, long captureTimeNanos){
        ...
        for (int i = 0; i < contours.size(); i++){
            ...
            if (rect.area() > 8000 && rect.area() < 15000){
        ...
        ArrayList<Point> centerPointsMulti = new ArrayList<>();
        ...
        for (int i = 0; i < redContoursMulti.size(); i++){
            centerPointsMulti.add(getContourCenter(redContoursMulti.get(i)));
        }

        ArrayList<MatOfPoint> sortedContoursSingle = sortContoursByY(redContoursSingle, centerPointsSingle);
        ArrayList<MatOfPoint> sortedContoursMulti = sortContoursByYMulti(redContoursMulti, centerPointsMulti);
        ...
        for (MatOfPoint contour : sortedContoursMulti) {
            Imgproc.drawContours(frameSub, Arrays.asList(contour), -1, new Scalar(0, 255, 255), 2);
        }

        if (sortedContoursSingle.size() > 0){

            MatOfPoint2f contour2f = new MatOfPoint2f(sortedContoursSingle.get(sortedContoursSingle.size()-1).toArray());
            ...
            Point contourCenter = getContourCenter(sortedContoursSingle.get(sortedContoursSingle.size()-1));

            double angleMultiplier = (0.9)*(Math.abs(angle))/100;

            double x = calculateAdjustment(contourCenter.y, 496, 0, 0, 32)*angleMultiplier;

            TargetPoint = new Point(contourCenter.x, contourCenter.y - x);

        } else if (sortedContoursMulti != null) {

            MatOfPoint closestContour = sortedContoursMulti.get(sortedContoursMulti.size()-1);
...
//                }
        ...
        Imgproc.putText(frameSub, String.valueOf(angle), new Point(20, 40), 2, 1, new Scalar(0, 255, 0), 4, 2, false);
